...
qsub -n pipe_${} -V -l time= 48:00:00 -cwd -shell n -b y -o ${logFolder}/${subID}_preproc.o ${rootPath}/preprocDK.sh ${subFolder}/ ${subID} > $jobFile
...
if [ -d "$subFolder/$subID/RAWDATA/BOLD-EPI" ]; then
	qsub -n fc_${subID} -V -hold_jid depend=afterok:${jobID} -cwd -shell n -b y -o ${logFolder}/${subID}_functional.o -l time=00:55:00 ${rootPath}/fmriFC.sh ${subFolder}/ ${subID} > $jobFile
	#Extract the Job ID from the previously submitted job
	jobID=$(tail -n 1 $jobFile)
	echo $jobID >> $jobListFile
fi
...
qsub -n Mask_${subID} -V -hold_jid depend=afterok:${jobID} -cwd -shell n -b y -r y -o ${logFolder}/${subID}_mask.o  -l time=01:00:00 ${rootPath}/genMaskDK.sh ${subFolder} ${subID} ${rootPath} > $jobFile
...
chmod +x *.sh
qsub -n trk_${subID} -V -hold_jid depend=afterok:${jobID} -cwd -shell n -b y -l nodes=192:ppn=16 -o ${logFolder}/${subID}_tracking.o -l walltime=03:30:00 ./runTracking.sh > $jobFile
