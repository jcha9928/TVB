...
recon-all -i ${path}/${pfx}/RAWDATA/MPRAGE/${firstFile} -subjid recon_all -sd ${path}/${pfx} -all
...
#Rotate high-res (1mm) T1.mgz to match dwi data (bbregister)
mri_convert brain.mgz brain_anat_orig.nii.gz
mri_convert T1.mgz T1.nii.gz
flip4fsl brain_anat_orig.nii.gz brain_anat.nii.gz
flip4fsl T1.nii.gz T1_flip.nii.gz
tkregister2 --mov brain_anat_orig.nii.gz --targ brain_anat.nii.gz --regheader --noedit --fslregout anatorig2anat.mat --reg anat2anatorig.dat 
convert_xfm -omat anat2anatorig.mat -inverse anatorig2anat.mat
bbregister --s recon-all --init-fsl --dti --mov $lowb --reg anatorig2diff.bbr.dat --fslmat diff2anatorig.bbr.mat
convert_xfm -omat anatorig2diff.bbr.mat -inverse diff2anatorig.bbr.mat
convert_xfm -omat anat2diff.bbr.mat -concat anatorig2diff.bbr.mat anat2anatorig.mat
convert_xfm -omat diff2anat.bbr.mat -inverse anat2diff.bbr.mat
flirt -in $lowb -ref $lowb -o lowb_iso -applyisoxfm 1
flirt -in brain_anat.nii.gz -ref lowb_iso -applyxfm -init anat2diff.bbr.mat -o brain2diff.nii.gz
...
5ttgen fsl brain2diff.nii.gz 5TT.mif -premasked
...
mrconvert ${path}/${pfx}/RAWDATA/DTI/ dwi1.mif
#Export the btable in MRTrix Format
#mrinfo ${path}/${pfx}/RAWDATA/DTI/ -grad btable.b

dwidenoise dwi1.mif -force dwi_denoised.mif

dwipreproc PA dwi_denoised.mif dwi_denoised_preproc.mif -rpe_none -force

dwi2mask dwi_denoised_preproc.mif - | maskfilter - erode -npass 7 -force mr_eroded_mask.mif

dwibiascorrect dwi_denoised_preproc.mif -force dwi_denoised_preproc_biasCorr.mif -ants -mask mr_eroded_mask.mif 

cp dwi_denoised_preproc_biasCorr.mif dwi.mif

#Diffusion tensor images
dwi2tensor dwi.mif dt.mif
#Fractional anisotropy (FA) map
tensor2metric dt.mif -fa fa.mif -adc adc.mif
...
mrcalc ev.mif fa_corr.mif -mult ev_scaled.mif

#Mask of single-fibre voxels
maskfilter -npass 1 wmmask.mif erode - | mrcalc fa_corr.mif - -mult - | mrthreshold -abs 0.7 - sf.mif
#Response function coefficient
#JIOOK's example 

dwi2response tournier -mask mr_eroded_mask.mif -voxels mr_voxels_reduced_eroded.mif dwi.mif response.txt -force
...
dwi2fod csd dwi.mif response.txt mr_fod.mif -mask wmmask.mif
